...
class _ValidateTypeState extends State<ValidateType> {
  ...
  void afterFirstLayout() {
    context.read<PreUploadContentNotifier>().initThumbnail().then((value) => getThumbnail().then((value) {
          setState(() {
            isLoading = false;
          });
        }));
  ...
  Future getThumbnail() async {
    ...
    _isImage = System().lookupContentMimeType(notifier.fileContent?[0]?.split('/').reversed.elementAt(0) ?? '')?.contains('image');

    isEditingOverlay = widget.editContent ? notifier.featureType == FeatureType.pic : (_isImage ?? false) || _isImage == null;
