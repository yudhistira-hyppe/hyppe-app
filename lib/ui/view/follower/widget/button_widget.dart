...
class ButtonWidget extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return (data?.eventType == InteractiveEventType.follower &&
                data?.event == InteractiveEvent.request) ||
            (data?.eventType == InteractiveEventType.following &&
                data?.event == InteractiveEvent.initial)
        ? CustomTextButton(
            onPressed: () {
              ...
              if (data != null) {
                if (data?.eventType == InteractiveEventType.follower &&
                  ...
                  context
                      .read<FollowerNotifier>()
                      .followUser(context, data: data, argument: argument);
                ...
                } else if (data?.event == InteractiveEvent.accept ||
                    data?.event == InteractiveEvent.done) {
                  argument = FollowUserArgument(
                    receiverParty: data?.senderOrReceiver ?? '',
                    eventType: InteractiveEventType.unfollow,
                  );
            ...
            child: CustomTextWidget(
              ...
              textStyle: Theme.of(context).textTheme.labelLarge?.copyWith(
                    color: data?.event != InteractiveEvent.accept
                        ? Colors.white
                        : null,
                  ),
            ),
  ...
  String? buttonText(BuildContext context) {
    ...
    } else if (data?.eventType == InteractiveEventType.follower &&
        data?.event == InteractiveEvent.request) {
