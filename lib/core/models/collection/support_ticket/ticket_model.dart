  ...
  TicketModel.fromJson(Map<String, dynamic> map) {
    ...
    if (map['fsTargetUri'] != null) {
      ...
      for (var i = 0; i < fsSourceName!.length; i++) {
        imageUrl.add('${Env.data.baseUrl}${Env.data.versionApi}${Env.data.versionApi == '' ? '/' : ''}ticket/detail/supportfile/$id/$i?x-auth-token=$token&x-auth-user=$emailUser');
      }
    }
    version = map['version'];
    os = map['OS'];
    ...
    if (map['fsTargetUri'] != null) {
      ...
      for (var i = 0; i < fsTargetUri!.length; i++) {
        ticketUrls.add(TicketUrl(
            localDir: fsTargetUri![i],
            realUrl: '${Env.data.baseUrl}${Env.data.versionApi}${Env.data.versionApi == '' ? '/' : ''}ticket/detail/supportfile/$id/$i?x-auth-token=$token&x-auth-user=$emailUser'));
      }
    }
