## 0.0.85+2
* Move `expandButton` position widget in `cupertino_controll` widget to `bottomBar` widget.

## 0.0.84+2
* Minor fixes `skipAdsButtonAt` event in `BetterPlayerCupertinoControls` and `BetterPlayerMaterialControls`.

## 0.0.84+1
* Remove `_advertiser` parameter from `_postShowingAdsVideo`.

## 0.0.84
* Added `fromJson` constructor in `BetterPlayerRoll` Object.

## 0.0.84
* Export `BetterPlayerAdsConfiguration` Object.

## 0.0.82
* Added ads video configuration, for ads video support `experimental`.
