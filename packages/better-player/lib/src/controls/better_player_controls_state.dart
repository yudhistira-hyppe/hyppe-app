...
abstract class BetterPlayerControlsState<T extends StatefulWidget> extends State<T> {
      ...
      child: Container(
        child: Column(
          children: [
            ...
            if (betterPlayerControlsConfiguration.reportFuntion != null)
              _buildMoreOptionsListRow(Icons.warning_amber_rounded, translations.reportThisContent, betterPlayerControlsConfiguration.reportFuntion as VoidCallback),
  ...
  /// Closing ads when user taps on skip button.
  void closeAds() {
    if (latestValue != null) {
      betterPlayerController?.closingAds(BetterPlayerRoll(
        rollDuration: latestValue?.duration?.inSeconds,
        rollUri: betterPlayerController?.betterPlayerDataSource?.url,
      ));
    }
  }
}
