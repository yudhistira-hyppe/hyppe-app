...
import '../configuration/better_player_ads_configuration.dart';
...
class BetterPlayerController {
  ...
  ///Overriden Size of the video which will be used instead of aspect ratio
  ///passed in configutarion.
  Size? _overriddenVideoSize;
  ...
  ///Overridden overlay Layout of the video screen
  Widget? _overriddenOverlay;
  ...
  ///Reactive value which emits stream when show skip ads button appeared
  ValueNotifier<bool> skipAdsEventValueNotifier = ValueNotifier(false);
  ...
  void _onVideoPlayerChanged() async {
    ...
    if (now - _lastPositionSelection > 500) {
      ...
      if (_betterPlayerControlsConfiguration.skipAdsButtonAt != null) {
        _showSkipAdsButton(currentVideoPlayerValue);
      } else {
        _showAds(currentVideoPlayerValue.position);
      }
    }
  }
  ...
  /// Show ads video.
  void _showAds(Duration position) {
    final _roll = _pickNextAds();

    if (_roll == null) {
      return;
    }

    final _currentPosition = position;

    if (_currentPosition.compareTo(_adsPositionCalculation(_roll.rollDuration ?? 0)) >= 0) {
      _postShowingAdsVideo(_roll);
    }
  }

  /// Closing ads video, usually after this method called remove ads video from original playlist video.
  void closingAds(BetterPlayerRoll? roll) {
    _postClosingAdsVideo(roll);
  }

  /// Show skip ads button.
  void _showSkipAdsButton(VideoPlayerValue currentVideoPlayerValue) {
    final _currentPosition = currentVideoPlayerValue.position;
    final _skipAdsButtonPosition = _adsPositionCalculation(_betterPlayerControlsConfiguration.skipAdsButtonAt ?? 0);
    final _roll = BetterPlayerRoll(
      rollUri: _betterPlayerDataSource?.url,
      rollDuration: currentVideoPlayerValue.duration?.inSeconds,
    );

    if (_currentPosition.compareTo(_skipAdsButtonPosition) > 0) {
      skipAdsEventValueNotifier.value = true;
      _postShowSkipAdsButton(_roll);
    }
  }

  /// Show skip ads button.
  void _postShowSkipAdsButton(BetterPlayerRoll? roll) {
    _postEvent(
      BetterPlayerEvent(
        BetterPlayerEventType.showSkipAds,
        parameters: roll?.toJson(),
      ),
    );
  }

  /// Showing ads video.
  void _postShowingAdsVideo(BetterPlayerRoll? roll) {
    _postEvent(
      BetterPlayerEvent(
        BetterPlayerEventType.showingAds,
        parameters: roll?.toJson(),
      ),
    );
  }
  ...
  /// Closing ads video.
  void _postClosingAdsVideo(BetterPlayerRoll? roll) {
    _postEvent(
      BetterPlayerEvent(
        BetterPlayerEventType.closingAds,
        parameters: roll?.toJson(),
      ),
    );
  }
  ...
  Duration _adsPositionCalculation(int inSecond) {
    int minutes = inSecond ~/ 60;
    int seconds = inSecond % 60;
    return Duration(minutes: minutes, seconds: seconds);
  }

  /// Pick next ads from queue.
  BetterPlayerRoll? _pickNextAds() {
    return _betterPlayerDataSource?.adsConfiguration?.rolls?.firstOrNull;
  }

  /// Delete ads by uri.
  void deleteAdsByUri(String uri) {
    _betterPlayerDataSource?.adsConfiguration?.rolls?.removeWhere((roll) => roll.rollUri == uri);
  }
  ...
  ///Setup overridden video size.
  void setOverridenVideoSize(Size videoSize){
    _overriddenVideoSize = videoSize;
  }

  ///Get video size used in current video. If video size is null, then
  ///video size from BetterPlayerConfiguration will be used. Otherwise
  ///[_overriddenVideoSize] will be used.
  Size? getVideoSize(){
    return _overriddenVideoSize ?? betterPlayerConfiguration.videoSize;
  }
  ...
  void setOverlay(Widget? widget){
    _overriddenOverlay = widget;
  }

  ///Get overlay layout
  Widget? getOverlay(){
    return _overriddenOverlay ?? betterPlayerConfiguration.overlay;
  }
